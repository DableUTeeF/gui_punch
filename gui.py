...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.to_broadband = QPushButton('Broad (B)')
        self.to_narrowband = QPushButton('Narrow (N)')
        self.to_None = QPushButton('None (O)')
        self.to_pulse_and_broad = QPushButton('Pulse and Broad')
        self.to_pulse_and_narrow = QPushButton('Pulse and Narrow')
        self.to_unknown = QPushButton('Unknown')

        layout_labelibg = QGridLayout()
        # addWidget(QWidget, row, column, rows, columns)
        layout_labelibg.addWidget(self.to_pulse, 0, 0, 1, 1)
        layout_labelibg.addWidget(self.to_None, 0, 1, 1, 1)
        layout_labelibg.addWidget(self.to_broadband, 1, 0, 1, 1)
        layout_labelibg.addWidget(self.to_narrowband, 1, 1, 1, 1)
        layout_labelibg.addWidget(self.to_pulse_and_broad, 2, 0, 1, 1)
        layout_labelibg.addWidget(self.to_pulse_and_narrow, 2, 1, 1, 1)
        layout_labelibg.addWidget(self.to_unknown, 3, 0, 1, 2)
        ...
        self.to_broadband.setEnabled(False)
        self.to_narrowband.setEnabled(False)
        ...
        self.to_broadband.clicked.connect(lambda: self.set_label('Broad'))
        self.to_broadband.setShortcut("I")
        self.to_broadband.setToolTip("Hotkey: i")
        self.to_None.clicked.connect(lambda: self.set_label('None'))
        self.to_None.setShortcut("O")
        self.to_None.setToolTip("Hotkey: o")
        self.to_pulse_and_broad.clicked.connect(
            lambda: self.set_label('Pulse and Broad')
        )
        self.to_pulse_and_narrow.clicked.connect(
            lambda: self.set_label('Pulse and Narrow')
        )
        self.to_unknown.clicked.connect(
            lambda: self.set_label('Unknown')
        )
    ...
    def start_labeling(self):
        ...
        if self.directory:
            ...
            self.to_broadband.setEnabled(True)
            self.to_narrowband.setEnabled(True)
            self.to_None.setEnabled(True)
            self.to_pulse_and_broad.setEnabled(True)
            self.to_pulse_and_narrow.setEnabled(True)
            self.to_unknown.setEnabled(True)
